...
public:
	...
	// adhoc
	DX12DescriptorChunk allocate_cbv_srv_uav_cpu(uint32_t num_descriptors);
	void free_cbv_srv_uav_cpu(DX12DescriptorChunk* chunk);
...
private:
	std::unique_ptr<DX12DescriptorHeap> m_cpu_dh_resource;		// CBV_SRV_UAV 
	...
	std::unique_ptr<DX12DescriptorAllocatorDMA> m_cpu_dh_resource_ator;
