...
void PCGLevelScene::SetUpScene(std::vector<std::function<std::vector<DOG::entity>()>> entityCreators)
{
	...
	uint32_t enemySpawnRarity = 5u; // Spawns enemies once every X blocks.
	...
	std::uniform_int_distribution<uint32_t> enemyNrDist(1u, maxEnemiesPerSpawn);
	std::uniform_real_distribution<float> enemySpreadDist(0.5f, 2.5f);
	...
	EntityManager::Get().Collect<FloorBlockComponent>().Do([&](entity e, FloorBlockComponent&)
		{
			...
			if (enemyCounter % enemySpawnRarity == 0)
			{
				...
				{
					//Spawn enemies
					AddEntities(m_spawnAgents(EntityTypes::Scorpio, Vector3(pos.x + 2.5f, pos.y + 5.0f, pos.z + 2.5f), (u8)(enemyNrDist(gen)), enemySpreadDist(gen)));
				}
			}
			//Spawn items
			ItemManager::Get().CreateItemHost(EntityTypes(((u32)itemCounter) % (u32(EntityTypes::Default) * itemSpawnModifier)), Vector3(pos.x + 2.5f, pos.y + 1.0f, pos.z + 2.5f));
			++enemyCounter;
			++itemCounter;
		});
}
