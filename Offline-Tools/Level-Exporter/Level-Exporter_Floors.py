...
def map_analysis():
    ...
    for (x, y, z), block_id in all_blocks(): #Go through all blocks in the input level.
        ...
        add_to_dict(blockDict, block_id, xPlus, xMinus, yPlus, yMinus, zPlus, zMinus)
        ...
        for i in range(1, 4):
            ...
            add_to_dict(blockDict, newName, xPlus, xMinus, yPlus, yMinus, zPlus, zMinus)
...
def add_to_dict(blockDict, block_id, xPlus, xMinus, yPlus, yMinus, zPlus, zMinus):
    if block_id not in blockDict: #If the current block is not in the dictionary yet we create an entry for it.
        ...
        #IN ENGINE WE USE LEFT HAND! Y AND Z CHANGES PLACE!
            ...
            add_rotations(blockDict, block_id, xPlus, 1)
            add_rotations(blockDict, block_id, xMinus, 2)
            add_rotations(blockDict, block_id, yPlus, 3)
            add_rotations(blockDict, block_id, yMinus, 4)
            add_rotations(blockDict, block_id, zPlus, 5)
            add_rotations(blockDict, block_id, zMinus, 6)
    ...
    if block_id == "Floor1_r0_f":
        block_id = rotate_string(block_id)
        add_to_dict(blockDict, block_id, xPlus, xMinus, yPlus, yMinus, zPlus, zMinus)
        block_id = rotate_string(block_id)
        add_to_dict(blockDict, block_id, xPlus, xMinus, yPlus, yMinus, zPlus, zMinus)
        block_id = rotate_string(block_id)
...
def add_rotations(blockDict, block_id, dir, index):
    if dir == "Floor1_r0_f":
        temp = dir
        for i in range(1, 4):
            temp = rotate_string(temp)
            if blockDict[block_id][index].count(temp) == 0:
                blockDict[block_id][index].append(temp)
